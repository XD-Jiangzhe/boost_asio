...
using chat_participant_ptrgroup = std::set<chat_participant_ptr>;
using ptrgroup_ptr = std::shared_ptr<chat_participant_ptrgroup>;
using mutex_lockguard = std::lock_guard<std::mutex>;
//-------------------------------------------------------
class chat_room{
public:
    chat_room():mutex_(), par_group_tr(new chat_participant_ptrgroup()), mutex_mesgs(){}
    void join(chat_participant_ptr p)
    {
        mutex_lockguard lg(mutex_);
        if(!par_group_tr.unique())
        {
            par_group_tr.reset(new chat_participant_ptrgroup(*par_group_tr));       //这里拷贝出一份副本用来修改
            BOOST_LOG_TRIVIAL(info)<<"LINE "<<__LINE__<<" "<<"copy the whole session successfully";
        }
        assert(par_group_tr.unique());

        par_group_tr->insert(p);
        {
            mutex_lockguard lg(mutex_mesgs);
    ...
    void leave(std::string name, chat_participant_ptr p)
    {
        ...
        if(!par_group_tr.unique())
        {
            par_group_tr.reset(new chat_participant_ptrgroup(*par_group_tr));       
            BOOST_LOG_TRIVIAL(info)<<"LINE "<<__LINE__<<" "<<"copy the whole session successfully";           
        }
        assert(par_group_tr.unique());
        par_group_tr->erase(p);
    }
...
private:
    ...
    std::mutex mutex_;
    ptrgroup_ptr par_group_tr;

    std::mutex mutex_mesgs;
...
class chat_server{
    public:
        chat_server(boost::asio::io_service &io, const tcp::endpoint &end_point):
                socket_(io), acceptor_(io, end_point), s_strand(io)
                {
                   do_accept();
                }
    private:
