            ...
            if(!err)
             {
                ...
                {
                    // const Roominformation *info = (Roominformation*)(read_mesg.body());
                    // std::cout<<"client: ";
                    // assert(sizeof(info->name.name)>= info->name.nameLen);
                    // std::cout.write(info->name.name, info->name.nameLen);
                    // std::cout<<" says ";
                    // assert(sizeof(info->chat.infomation) >= info->chat.infoLen);
                    // std::cout.write(info->chat.infomation, info->chat.infoLen);
                    // std::cout<<" \n"; 
                    std::stringstream ss(std::string(read_mesg.body(), read_mesg.body()+read_mesg.body_length()));
                    boost::archive::text_iarchive to(ss);   //这里是从ss中读入
                    SRoomInfo info;
                    to & info;         //load 出一个info结构体
                    cout<<"client: "<<info.name()<<" says "<<info.information()<<endl;
                }
                 do_read_header();
             }else 
                socket_.close();
...
int main(int argc, char* argv[])
{
    try{
        ...
        {
            chat_message mesg;
            // mesg.body_length(std::strlen(line));
            // std::memcpy(mesg.body(), line, mesg.body_length());
